...
public class Aldg extends Graph{
	...
	public Aldg(int fileType, int numNodes , int numEdges , String edges) throws FileNotFoundException{
		...
		for(int i = 0; i < edgeDetermineArray.length; i = i + 2){
			adjacencyList[edgeDetermineArray[i]].add(new Edge(edgeDetermineArray[i], edgeDetermineArray[i + 1]));
		}
		assignment2TestData();
	...
	protected void putEdge(int i, int j) {
		adjacencyList[i].add(new Edge(i,j));
	}
	...
	protected void printAdjacencyList(){
		for(int i = 0; i < adjacencyList.length; i++){
			...
			while(ite.hasNext()){
				System.out.println(i + " --> " + ite.next().vertex2 + " --> ");
			}
		}
	}
}
